...
/**
 * Updates the role grants for a resource. Unlike OrganizationAndProjectService's updateResourceRoles endpoint,
 * this endpoint uses the new `gk:all-without-mandatory` operation instead of the `gk:all` operation to
 * disable inherited permissions.
 *
 * The disable inherited permissions statements generated by OrganizationAndProjectService's updateResourceRoles endpoint,
 ...
 */
export async function updateResourceRolesV2(
  ctx: ConjureContext,
  rid: string,
  request: UpdateResourceGrantsRequest,
): Promise<void> {
  return conjureFetch(ctx, `/roles/v2/${rid}`, "POST", request);
}
