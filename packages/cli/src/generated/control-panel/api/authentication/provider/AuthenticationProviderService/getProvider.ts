...
/**
 * Returns the provider with the given RID. Requires `control-panel:authentication-provider:view` on that provider.
 */
export async function getProvider(
  ctx: ConjureContext,
  providerRid: AuthenticationProviderRid,
): Promise<AuthenticationProvider> {
  return conjureFetch(ctx, `/authentication-provider/${providerRid}`, "GET");
}
