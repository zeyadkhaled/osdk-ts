...
/**
 * Fetches the last saved user intake configuration associated with a provider.
 * Requires `control-panel:authentication-provider:view` on the provider.
 */
export async function getLatestUserIntakeConfiguration(
  ctx: ConjureContext,
  providerRid: AuthenticationProviderRid,
): Promise<UserIntakeConfiguration | undefined> {
  return conjureFetch(
    ctx,
    `/user-intake/v2/providers/${providerRid}/latest`,
    "GET",
  );
}
